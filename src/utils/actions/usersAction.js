...
export default async function action({ request }) {
    let url = `${import.meta.env.VITE_URL}/api/users/`;
    ...
    requestFormData.password = data.get("password")
        ? data.get("password")
        : undefined;
    requestFormData.active =
        typeof data.get("status") === "string"
            ? data.get("status").length === 0
                ? undefined
                : data.get("status").toLowerCase() === "active"
            : undefined;
