export default function workshopsToCards(users, committees, workshops) {
    return workshops.map((workshop) => {
        const cardFormFields = Object.keys(workshop)
            .map((field) => {
                if (field.toLowerCase() === "prerequisites") {
                    return {
                        label: field.toLowerCase(),
                        defaultValue:
                            workshop[field].length > 0
                                ? workshop[field].join(",")
                                : "",
                        placeholder:
                            "enter prerequisites each seperated by a comma",
                    };
                } else if (
                    field.toLowerCase() === "id" ||
                    field.toLowerCase() === "committee" ||
                    field.toLowerCase() === "instructor"
                ) {
                    return undefined;
                    ...
                    return {
                        label: field.toLowerCase(),
                        options: ["not-started", "finished", "in-progress"],
        ...
        if (committees) {
            cardFormFields.push({
                ...
                inputType: "dropdown",
                defaultValue: workshop.Committee && workshop.Committee.Title,
            });
        }
        if (users) {
            cardFormFields.push({
                ...
                inputType: "dropdown",
                defaultValue:
                    workshop.Instructor && workshop.Instructor.Username,
            });
        }
        return {
