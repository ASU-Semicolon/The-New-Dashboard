...
function Committees() {
    ...
    const filteredCommittees = committeesData.filter((committee) => {
        if (committee.Season === selectedYear) {
            return (
                committee.Title &&
                committee.Title.toLowerCase().includes(
    ...
    useHandleData(committeeData, {
        addData: addCommittee,
        ...
        <>
            <Modal setShowModal={setShowModal} showModal={showModal}>
                <ModalForm
                    ...
                    title="Add Committee"
                    fieldsArr={addCommitteeFormData(sectorsData)}
                />
            </Modal>
            <main className="section__committees">
                <div className="title__cont">
                    <h1 className="primary__title">Committees</h1>
                    <div className="add_committee__cont">
                        <Button
                            select="primary"
                            disabled={!isAdmin || isLoadingSectors}
                            onClick={() => {
                                setShowModal(true);
                            }}
                            rounded={false}
                ...
                {isLoading ? (
                    ...
                    <CardGrid
                        cardSize="small"
                        disableButtons={!isAdmin || isLoadingSectors}
                        gridSize="big"
                        cardFormTitle="Edit Committee"
                        cardFormButtonText="Edit Committee"
                        fallbackText="No Committees Found."
                        cards={committeesToCards(
                            filteredCommittees,
                            sectorsData,
                        )}
                    />
